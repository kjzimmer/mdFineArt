...
**Starting a work before it's finished:** Click "Add Work" and set Status to "In Progress" — no title or image is required yet when that status is selected. While a work is In Progress, its editor shows two extra sections: Progress Photos (snapshots of the piece as it develops — always a fresh upload, never picked from the library, since each one is a one-off moment) and Reference Photos (visual reference material used while painting — pick an existing photo from the Reference Library or upload a new one directly, either way it lands in the shared library for reuse on other works). A "Show on landing page" checkbox (same field as "Show in public gallery" on a finished work) controls whether this piece appears in the home page's Works in Progress section. Requires Works in Progress enabled in Configuration → Site Features. In-progress works, along with Featured works, get their own grouping at the top of the Works admin list for quick access; there's also a search-by-title box.
...
Shows a "Works in Progress" section on the public home page — every in-progress work with "Show on landing page" checked, most recently active one first (based on when its latest progress photo was added, not when the work record itself was last edited). Each one shows its progress photos as individual thumbnails (not a slideshow) and, if a current/completed image has been set, that image alongside it. Click any thumbnail to open a full-screen viewer with zoom and pan, starting at that photo. There's no separate page for this — it's all on the home page.
