...
- All public URLs (og:url, JSON-LD, sitemap.xml, robots.txt) are built from `canonicalBaseUrl()` — the gallery's stored `customDomain`/`previewDomain` — never from the request's Host header. Matters once client domains actually route through the Cloudflare Worker: Railway sees `Host: fallback.mygalleryworks.com` in that path (the real domain only survives in `X-Gallery-Hostname`, used for gallery *lookup* but not previously for URL construction); melodydebenedictis.com only worked by coincidence today because her domain is temporarily a direct Railway custom domain, not yet on Worker routing
