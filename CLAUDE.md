...
- Public — Home page gets a "Works in Progress" section (gated by `SiteConfig.worksInProgressEnabled`, default off) showing *every* eligible in-progress work, most recently active first — ranked by latest progress-photo upload, not `Work.updatedAt` (a work's own row rarely changes once created; new photos are the real activity signal). Each work shows its progress-photo slideshow (smaller, right, full image visible via `SlideshowDisplay`'s new opt-in `fit="contain"` prop — default stays `"cover"` so hero/Commission/Classes slideshows are unaffected) alongside its current/completed image if one's been set (larger, left, `object-contain` in a fixed-height box so every work's card is a consistent size regardless of source image dimensions). First cut was a separate `/works-in-progress` page; replaced same session, before shipping, per Karl's call to simplify to a landing-page section with no separate page
